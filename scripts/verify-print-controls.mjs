...
try {
  ...
  for (const width of [320, 390, 760, 761, 1118]) {
    ...
    for (const lang of ['ja', 'en']) {
      ...
      assert.equal(await page.locator('.resume-print-desktop').isVisible(), !mobile);
      if (mobile) {
        assert.equal(await panel.isVisible(), false);
        sizes.push(await trigger.boundingBox());
        await trigger.focus();
        await page.keyboard.press('Enter');
        await page.waitForFunction(() => document.querySelector('.resume-print-trigger').getAttribute('aria-expanded') === 'true');
        assert.equal(await trigger.getAttribute('aria-expanded'), 'true');
        const bounds = await panel.boundingBox();
        assert.ok(bounds.x >= 0 && bounds.x + bounds.width <= width, 'orientation choices fit viewport');
        await page.keyboard.press('Tab');
        assert.equal(await panel.locator('button').first().evaluate((el) => el === document.activeElement), true);
      ...
      for (const [index, orientation] of ['portrait', 'landscape'].entries()) {
        ...
        const choices = page.locator(mobile ? '.resume-print-options button' : '.resume-print-desktop button');
        await choices.nth(index).click();
        assert.equal(await page.evaluate(() => document.documentElement.dataset.printOrientation), orientation);
        assert.match(await page.locator('#print-orientation-style').textContent(), new RegExp(`size: A4 ${orientation}`));
        assert.equal(await page.evaluate(() => window.printRequests), index + 1);
        if (mobile) assert.equal(await panel.isVisible(), false);
      }
      await page.emulateMedia({ media: 'print' });
      assert.equal(await trigger.isVisible(), false);
      assert.equal(await page.locator('.resume-print-desktop').isVisible(), false);
  ...
  assert.equal(await page.locator('.project-detail-links .e-solid-link').getAttribute('href'), 'https://architect.morimizu.dev/');
  assert.deepEqual(errors, []);
  console.log('PASS: bilingual mobile disclosure, keyboard/outside dismissal, portrait/landscape requests, desktop controls, print media hiding, app URL. OS print dialog not automated.');
} finally {
  await browser.close();
}
